...
-- Technical documents use a dedicated private bucket and policies so existing
-- generic storage policies can evolve independently.
insert into storage.buckets (id, name, public)
values ('documents', 'documents', false)
on conflict (id) do update set public = false;

drop policy if exists "technical documents storage read" on storage.objects;
create policy "technical documents storage read" on storage.objects for select using (
  bucket_id = 'documents'
  and (storage.foldername(name))[1] = public.get_current_user_company_id()::text
);
drop policy if exists "technical documents storage insert" on storage.objects;
create policy "technical documents storage insert" on storage.objects for insert with check (
  bucket_id = 'documents'
  and (storage.foldername(name))[1] = public.get_current_user_company_id()::text
);
drop policy if exists "technical documents storage update" on storage.objects;
create policy "technical documents storage update" on storage.objects for update using (
  bucket_id = 'documents'
  and (storage.foldername(name))[1] = public.get_current_user_company_id()::text
) with check (
  bucket_id = 'documents'
...
create policy "technical documents storage delete" on storage.objects for delete using (
  bucket_id = 'documents'
  and (storage.foldername(name))[1] = public.get_current_user_company_id()::text
);
