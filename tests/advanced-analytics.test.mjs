...
import { buildAdvancedAnalyticsSnapshot, safePercent, toNumber } from "../lib/analytics/advanced-dashboard.ts";
...
test("advanced analytics snapshot computes conversion, delay, receivable, and risk edge cases", () => {
  ...
  const snapshot = buildAdvancedAnalyticsSnapshot({
    ...
    dispatches: [
      { id: "d1", dispatch_date: "2026-06-09", total_weight_kg: 700, delivery_status: "delivered" },
      { id: "d2", dispatch_date: "2026-06-11", total_weight_kg: 450, delivery_status: "delayed" },
    ],
    invoices: [
      {
        id: "i1",
    ...
    expenses: [
      { id: "e1", total_amount: 50000, payment_status: "paid", approval_status: "approved", invoice_date: "2026-06-03" },
      { id: "e2", total_amount: 30000, payment_status: "unpaid", approval_status: "pending_approval", invoice_date: "2026-06-04" },
    ],
    qualityInspections: [
  ...
  assert.equal(snapshot.kpis.quoteConversionRateMtd, 50);
  ...
  assert.equal(snapshot.kpis.expensePaidRateMtd, 62.5);
  assert.equal(snapshot.kpis.qualityPassRateMtd, 50);
  assert.equal(snapshot.kpis.inventoryAtRiskCount, 2);
  assert.equal(snapshot.kpis.inventoryOutOfStockCount, 1);
