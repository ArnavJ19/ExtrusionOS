...
export default async function DealerQuoteOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: order } = await supabase
    .from("orders")
    .select("*, dealers(dealer_name, dealer_code), customers(customer_name, company_name), quotes(quote_number), order_stage_history(id, stage, changed_at, remarks), order_dealer_stock_fulfillments(id, quote_item_id, finishing_type, unit_rate, fulfilled_weight_kg, fulfilled_length_m, created_at, aluminium_profiles(profile_code, profile_name), quote_items(item_description)), dispatches(id, dispatch_number, dispatch_date, delivery_status, total_weight_kg)")
    .eq("id", id)
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><h2 className="text-lg font-bold">Dealer Stock Fulfillment</h2></CardHeader><CardContent><div className="overflow-x-auto"><table className="industrial-table min-w-[960px]"><thead><tr><th>Profile</th><th>Quote Line</th><th>Finish</th><th>Rate</th><th>Weight</th><th>Length</th><th>Created</th></tr></thead><tbody>{(order.order_dealer_stock_fulfillments ?? []).map((row: any) => <tr key={row.id}><td>{row.aluminium_profiles?.profile_code ?? "Profile"}<p className="text-xs text-neutral-500">{row.aluminium_profiles?.profile_name ?? ""}</p></td><td>{row.quote_items?.item_description || (row.quote_item_id ? `Linked ${String(row.quote_item_id).slice(0, 8)}` : "Legacy line")}</td><td><Badge value={row.finishing_type ?? "stock"} /></td><td>{row.unit_rate ? `${formatCurrency(row.unit_rate)}/kg` : "-"}</td><td>{formatWeight(row.fulfilled_weight_kg)}</td><td>{Number(row.fulfilled_length_m ?? 0).toFixed(3)} m</td><td>{formatDate(row.created_at)}</td></tr>)}</tbody></table></div></CardContent></Card>
    </div>
  );
}
