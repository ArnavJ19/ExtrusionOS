...
export default async function MobileQualityPage() {
  ...
  const { data: checks } = await supabase
    .from("quality_tests")
    .select("id, test_number, test_type, status, result, inspector_name, production_jobs(job_number), aluminium_profiles(profile_code, profile_name)")
    ...
    <div className="mx-auto max-w-xl space-y-4">
      ...
      {checks?.length ? checks.map((check: any) => (
        <Link href={`/quality/${check.id}`} key={check.id} className="block rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition active:scale-[0.99]">
          <div className="flex items-start justify-between gap-3"><div><p className="text-xl font-black text-slate-950">{check.test_number}</p><p className="mt-1 text-sm font-medium text-slate-500">{check.aluminium_profiles?.profile_code || check.aluminium_profiles?.profile_name || check.production_jobs?.job_number || "Profile"} - {check.inspector_name || "Inspector not assigned"}</p></div><span className={`rounded-full px-3 py-1 text-xs font-black capitalize ${statusClass(check.status)}`}>{String(check.status || "pending").replace(/_/g, " ")}</span></div>
          <p className="mt-4 rounded-2xl bg-slate-50 p-3 text-sm font-bold text-slate-700"><Ruler className="mr-2 inline h-4 w-4 text-orange" />{String(check.test_type || check.result || "Inspection").replace(/_/g, " ")}</p>
