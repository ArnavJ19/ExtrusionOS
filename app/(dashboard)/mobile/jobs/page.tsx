...
export default async function MobileJobsPage() {
  ...
  const { data: jobs } = await supabase
    ...
    .select("id, job_number, current_stage, planned_quantity_kg, actual_output_kg, scrap_kg, priority, machines(machine_name), dies(die_number), orders(order_number, customers(customer_name, company_name)), aluminium_profiles(profile_code, profile_name)")
  ...
  return (
    <div className="mx-auto max-w-xl space-y-4">
      <MobileTitle icon={Factory} title="Today Jobs" subtitle="Open real production jobs to update status, output, scrap, and proof from the job detail workflow." />
      {jobs?.length ? jobs.map((job: any) => (
        <Link href={`/production/${job.id}`} key={job.id} className="block rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition active:scale-[0.99]">
          <div className="flex items-start justify-between gap-3"><div><p className="text-xl font-black text-slate-950">{job.job_number}</p><p className="mt-1 text-sm font-medium text-slate-500">{job.orders?.customers?.company_name || job.orders?.customers?.customer_name || job.orders?.order_number || "Customer"} - {job.aluminium_profiles?.profile_code || job.aluminium_profiles?.profile_name || "Profile"}</p></div><span className={`rounded-full px-3 py-1 text-xs font-black capitalize ${statusClass(job.priority)}`}>{String(job.priority || "normal").replace(/_/g, " ")}</span></div>
          <div className="mt-4 grid grid-cols-3 gap-2 text-center"><Mini label="Plan" value={formatWeight(job.planned_quantity_kg)} /><Mini label="Actual" value={formatWeight(job.actual_output_kg)} /><Mini label="Scrap" value={formatWeight(job.scrap_kg)} /></div>
          <div className="mt-4 rounded-2xl bg-slate-50 p-3 text-sm font-bold text-slate-700"><Gauge className="mr-2 inline h-4 w-4 text-orange" />{job.machines?.machine_name || "Machine not assigned"} - {String(job.current_stage || "planned").replace(/_/g, " ")} - {job.dies?.die_number || "No die"}</div>
