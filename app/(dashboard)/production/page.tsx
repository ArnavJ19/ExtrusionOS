...
export default async function ProductionPage() {
  ...
  const [allJobs, recentJobs, monthlyCompleted, finishing, overdueJobs, noMachine] = await Promise.all([
    ...
    supabase.from("production_jobs").select("id, job_number, status, planned_date, planned_quantity_kg, actual_quantity_kg, machine:machines(machine_name), order:orders(order_number, customers(customer_name, company_name)), profile:profiles(profile_code)").eq("company_id", cid).order("created_at", { ascending: false }).limit(5),
  ...
  return (
    <div className="space-y-2">
      <ProductionDashboard
        ...
        allJobs={allJobsData}
      />
      <ModuleOverviewClient moduleKey="production" context={context} canCreate={can(context.role, "create", "production")} canUpdate={can(context.role, "update", "production")} />
