...
const attentionStatuses = ["planned", "sent_to_vendor", "in_process", "received", "rejected", "completed"];
...
export default async function ProductionFinishingPage({ searchParams }: { searchParams: Promise<{ production_job_id?: string; order_id?: string }> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-4"><Metric label="Open Jobs" value={String(openRows.length)} /><Metric label="At Vendor / In Process" value={formatWeight(sentWeight)} /><Metric label="Rejected" value={formatWeight(rejectionWeight)} /><Metric label="Total Jobs" value={String(rows.length)} highlight /></div>
